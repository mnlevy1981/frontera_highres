...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   integer (int_kind), public :: &
      ...
      tavg_TEMP_BOT,     &! tavg id for temperature at bottom cell
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_TEMP_BOT,'TEMP_BOTTOM',2,                      &
                          long_name='Potential Temperature at ocean floor',   &
                          units='degC', grid_loc='2111',                      &
                          coordinates='TLONG TLAT time')
...
!-----------------------------------------------------------------------
         ...
         ! (and find temp at ocean floor)
         where(k == KMT(:,:,iblock))
            WORK1 = TRACER(:,:,k,1,curtime,iblock)
         end where
...
!-----------------------------------------------------------------------
      ...
      enddo  ! k loop
      call accumulate_tavg_field(WORK1,tavg_TEMP_BOT,iblock,1)
